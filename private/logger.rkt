...
(define (log-args id args kws kw-vals level marks)
  (define str
    (match (expression-identifier->string id)
      [(? string? v)  v]
      [_ (~a `(,(syntax-e id) ,@args ,@(append-map list kws kw-vals)))]))
  (log! (~a (make-string (add1 level) #\>) " " str)
        (make-logger-event-value 'call id str level marks)))

(define (log-results id results level marks)
  (define str
    (~a (match results
          [(list)   "#<void>"]
          [(list v) (~v v)]
          [vs       (~s (cons 'values vs))])))
  (log! (~a (make-string (add1 level) #\<) " " str)
        (make-logger-event-value 'results id str level marks)))

(define (make-logger-event-value kind id str level marks)
